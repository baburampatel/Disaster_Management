...
def user_dashboard():
    ...
    recommendation = weak_topic = next_exercise = None
    learning_path = []
    if percentages:
        min_score = min(percentages)
        idx = percentages.index(min_score)
        ...
        nex = 1
        while nex in done_ex:
            nex += 1
        if nex <= 3:
            next_exercise = {'disaster': weak_topic, 'exercise': nex}
        sorted_pairs = sorted(zip(disasters, percentages), key=lambda x: x[1])
        learning_path = [{'disaster': d, 'score': round(s, 1)} for d, s in sorted_pairs[:3]]
    ...
    row = cur.fetchone()
    user_name = row[0] if row else session.get('user_name', 'User')
    db.close()
    return render_template("user_dashboard.html", disasters=disasters,
                           percentages=percentages, recommendation=recommendation,
                           notifications=notifications, weak_topic=weak_topic,
                           next_exercise=next_exercise, learning_path=learning_path,
                           user_name=user_name)
...
def feedback_page():
    if request.method == "POST":
        ...
        db = get_db()
        cur = db.cursor()
        cur.execute("INSERT INTO feedback (user_id,message) VALUES (?,?)", (session['user_id'], msg))
        db.commit()
        db.close()
...
def admin_dashboard():
    ...
    users = cur.fetchall()
    cur.execute("SELECT f.id,u.name,f.message,f.admin_reply FROM feedback f JOIN users u ON f.user_id=u.id WHERE f.admin_reply IS NULL")
    feedbacks = cur.fetchall()
    cur.execute("SELECT u.name,s.disaster_type,s.exercise_number,s.score,s.total,s.taken_at FROM disaster_scores s JOIN users u ON s.user_id=u.id ORDER BY s.taken_at DESC")
    scores = cur.fetchall()
    cur.execute("SELECT d.disaster_type,COALESCE(ROUND(AVG((s.score*1.0/s.total)*100),2),0) FROM disaster_details d LEFT JOIN disaster_scores s ON d.disaster_type=s.disaster_type GROUP BY d.disaster_type")
    analytics = cur.fetchall()
    analytics_labels = [a[0] for a in analytics]
    analytics_values = [float(a[1]) for a in analytics]
    total_users = len(users)
    cur.execute("SELECT COUNT(*) FROM users WHERE last_login >= datetime('now', '-7 days') AND role='user'")
    active_users = cur.fetchone()[0]
    weak_categories = [a[0] for a in analytics if float(a[1]) < 60]
    cur.execute("SELECT COUNT(*) FROM alerts WHERE is_active=1")
    active_alerts = cur.fetchone()[0]
    cur.execute("SELECT COUNT(*) FROM incident_reports WHERE status='pending'")
    pending_incidents = cur.fetchone()[0]
    db.close()
    return render_template("admin_dashboard.html", users=users, feedbacks=feedbacks,
                           scores=scores, analytics_labels=analytics_labels,
                           analytics_values=analytics_values, total_users=total_users,
                           active_users=active_users, pending_feedback=len(feedbacks),
                           quiz_count=len(scores), weak_categories=weak_categories,
                           active_alerts=active_alerts, pending_incidents=pending_incidents)
...
@login_required
def continents():
    continent_list = ["Asia","Africa","Europe","North America","South America","Australia","Antarctica"]
    return render_template("continents.html", continents=continent_list)
...
def disaster_details_page(disaster):
    db = get_db()
    cur = db.cursor()
    cur.execute("SELECT description,causes,impacts,case_study,lessons,dos,donts FROM disaster_details WHERE disaster_type=?", (disaster,))
    details = cur.fetchone()
    cur.execute("SELECT warning_signs,safety_kit,myths_facts,region_tips FROM disaster_articles WHERE disaster_type=?", (disaster,))
...
def simulation_exercise(disaster, exercise):
    db = get_db()
    cur = db.cursor()
    cur.execute("SELECT * FROM disaster_questions WHERE disaster_type=? AND exercise_number=?", (disaster, exercise))
    ...
    if request.method == "POST":
        ...
        for q in questions:
            ...
            if user_answer == correct:
                ...
                status = "wrong"
            review.append({"question": q['question'], "user_answer": user_answer,
                           "correct_answer": correct, "status": status})
        cur.execute("INSERT INTO disaster_scores (user_id,disaster_type,exercise_number,score,total) VALUES (?,?,?,?,?)",
...
def disaster_library():
    db = get_db()
    cur = db.cursor()
    cur.execute("SELECT disaster_type,description,continent,state FROM disaster_details GROUP BY disaster_type")
    disasters = cur.fetchall()
    db.close()
...
def disaster_guide(disaster_type):
    db = get_db()
    cur = db.cursor()
    cur.execute("SELECT * FROM disaster_details WHERE disaster_type=? LIMIT 1", (disaster_type,))
    details = cur.fetchone()
    cur.execute("SELECT * FROM disaster_articles WHERE disaster_type=? LIMIT 1", (disaster_type,))
    article = cur.fetchone()
    cur.execute("SELECT name,number,category FROM emergency_contacts WHERE category IN ('national','disaster','medical') LIMIT 6")
    contacts = cur.fetchall()
    db.close()
    return render_template("disaster_guide.html", disaster=disaster_type, details=details,
                           article=article, contacts=contacts)
...
def shelters():
    db = get_db()
    cur = db.cursor()
    cur.execute("SELECT * FROM shelters ORDER BY is_active DESC, state")
    shelter_list = cur.fetchall()
    db.close()
...
def report_incident():
    if request.method == 'POST':
        ...
        cur.execute("INSERT INTO incident_reports (user_id,incident_type,location,description,severity,contact_number) VALUES (?,?,?,?,?,?)",
                    (session['user_id'], incident_type, location, description, severity, contact_number))
        db.commit()
        db.close()
        flash("Incident reported successfully. Authorities have been notified.", "success")
...
def preparedness_checklist():
    db = get_db()
    cur = db.cursor()
    cur.execute("SELECT * FROM preparedness_checklists ORDER BY category, priority DESC, item")
    items = cur.fetchall()
    checklist = {}
    for item in items:
        cat = item['category']
...
def training():
    ...
    cur = db.cursor()
    cur.execute("SELECT DISTINCT disaster_type FROM disaster_details")
    disasters = [d[0] for d in cur.fetchall()]
    ...
    completed = {(r[0], r[1]): r[2] for r in cur.fetchall()}
    db.close()
...
def admin_content_manager():
    ...
    cur.execute("SELECT * FROM emergency_contacts ORDER BY category, name")
    contacts = cur.fetchall()
    cur.execute("SELECT id,incident_type,location,severity,status,reported_at FROM incident_reports ORDER BY reported_at DESC")
    incidents = cur.fetchall()
    db.close()
    return render_template("admin_content_manager.html", alerts=alerts,
                           contacts=contacts, incidents=incidents)
...
def admin_add_alert():
    ...
    cur.execute("INSERT INTO alerts (title,message,severity,location) VALUES (?,?,?,?)",
                (request.form['title'], request.form['message'],
                 request.form['severity'], request.form.get('location', '')))
    db.commit()
    db.close()
    flash("Alert published successfully.", "success")
    return redirect('/admin/content-manager')
...
def admin_add_contact():
    ...
    cur.execute("INSERT INTO emergency_contacts (name,number,category,description,region) VALUES (?,?,?,?,?)",
                (request.form['name'], request.form['number'], request.form['category'],
                 request.form.get('description', ''), request.form.get('region', 'All India')))
    db.commit()
    db.close()
...
def admin_analytics_expanded():
    db = get_db()
    cur = db.cursor()
    cur.execute("SELECT d.disaster_type, COALESCE(ROUND(AVG((s.score*1.0/s.total)*100),2),0), COUNT(DISTINCT s.user_id) FROM disaster_details d LEFT JOIN disaster_scores s ON d.disaster_type=s.disaster_type GROUP BY d.disaster_type")
    disaster_stats = cur.fetchall()
    cur.execute("SELECT u.name,s.disaster_type,s.exercise_number,s.score,s.total,s.taken_at FROM disaster_scores s JOIN users u ON s.user_id=u.id ORDER BY s.taken_at DESC LIMIT 15")
    recent_activity = cur.fetchall()
    cur.execute("SELECT DATE(taken_at),COUNT(*) FROM disaster_scores WHERE taken_at >= datetime('now','-7 days') GROUP BY DATE(taken_at) ORDER BY DATE(taken_at)")
    daily_activity = cur.fetchall()
    db.close()
    return render_template("admin_analytics.html", disaster_stats=disaster_stats,
                           recent_activity=recent_activity, daily_activity=daily_activity)
